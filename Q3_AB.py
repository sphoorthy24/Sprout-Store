...
def main():
	...
	model = LogisticRegression(lr=0.01, num_iter=300)
	model.fit(feature, np.array(target))

	preds = model.predict(feature,0.5)
	plt.scatter(h, np.linspace(np.min(h),np.max(h),len(h)))
	plt.scatter(w, np.linspace(np.min(w),np.max(w),len(w)))
	plt.scatter(a, np.linspace(np.min(a),np.max(a),len(a)))
